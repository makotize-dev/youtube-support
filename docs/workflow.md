...
X (Twitter) は **音楽著作権の包括契約が他プラットフォームほど整備されていない**。BGM 入りの動画クリップは DMCA 削除のリスクが相対的に高い。

#### 可否判定

| 管理団体 | X可否 |
|---|---|
| JASRAC / NexTone | `条件付き`（包括契約は一部のみ。短時間クリップなら問題になりにくい） |
| 両方 | `条件付き` |
| なし | `条件付き` |
| 不明 | `不明` |

> **X の現実的扱い：** 30秒〜1分程度の切り抜きであれば問題化することは稀。ただし保証はない。長尺は避ける。
...
| 公式音源使用・1分未満 | **低〜中** |
| MIDI起こし・耳コピ・1分未満 | **低** |
| 公式音源加工・長尺 | **高**（DMCA 削除の可能性） |
| 不明 | **不明** |

> **Xリスクの中身：** DMCA 削除 / 繰り返しでアカウント凍結リスク
...
「3プラットフォームすべて投稿可能」を GO の基準とする。

| 状況 | 自動判定 |
|---|---|
| 全プラットフォーム可・全リスク低・インスト確保済 | `GO` |
| 一部プラットフォームが不可 / リスク高 / インスト未確保 | `NG` |
| 不明な部分が残る | `保留` |

> **一部プラットフォームだけNGのとき：** たとえば「YouTube/TikTok は可、X だけリスク高」の場合、`自動判定=保留`（or NG）にして友人に判断を委ねる。備考欄に状況を明記。
